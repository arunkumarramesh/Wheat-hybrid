...
out_cds="CDS.bed"
out_prom="promoter1kb.bed"

awk -v OFS="\t" -v out_cds="$out_cds" -v out_prom="$out_prom" '
BEGIN { FS = "[[:space:]]+" }
...
$3 == "mRNA" || $3 == "transcript" {
    ...
    if (tx_id != "" && parent_gene_v11 != "") {
        tx_to_gene_v11[tx_id] = parent_gene_v11
    }
...
END {
    ...
    for (gene in best_tx) {
        ...
        for (i = 1; i <= n_cds[tx]; i++) {
            print cds_line[tx, i] > out_cds
        }
    }
}
' "$map" "$gff"
