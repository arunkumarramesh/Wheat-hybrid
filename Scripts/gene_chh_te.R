...
dt$distance_to_gene <- as.numeric(dt$distance_to_gene)
...
diff_summary_all <- rbindlist(list(make_diff_summary(dt, "subgenome", "subgenome"), make_diff_summary(dt, "te_class", "te_class"), make_diff_summary(dt, "te_status", "te_status"), make_diff_summary(dt, "centromere_status", "centromere_status"), make_diff_summary(dt, "distance_to_gene_class", "distance_to_gene_class")), use.names = TRUE, fill = TRUE)
...
dt <- dt[distance_to_gene < 1000]

dt[, meth_CS := pct_CS * cov_CS / 100]
dt[, meth_CSxP := pct_CSxP * cov_CSxP / 100]
dt[, meth_P := pct_P * cov_P / 100]

gene_meth <- dt[, .(meth_CS = sum(meth_CS, na.rm = TRUE), cov_CS = sum(cov_CS, na.rm = TRUE), meth_CSxP = sum(meth_CSxP, na.rm = TRUE), cov_CSxP = sum(cov_CSxP, na.rm = TRUE), meth_P = sum(meth_P, na.rm = TRUE), cov_P = sum(cov_P, na.rm = TRUE), n_TEs = uniqueN(te_id), n_sites = sum(n_sites, na.rm = TRUE)), by = nearest_gene_id_v2.1]

gene_meth[, pct_CS := 100 * meth_CS / cov_CS]
gene_meth[, pct_CSxP := 100 * meth_CSxP / cov_CSxP]
gene_meth[, pct_P := 100 * meth_P / cov_P]

gene_meth <- gene_meth[, .(gene_id = nearest_gene_id_v2.1, n_TEs, n_sites, pct_CS, cov_CS, pct_CSxP, cov_CSxP, pct_P, cov_P)]
gene_meth <- gene_meth[cov_CS > 10 & cov_CSxP > 10 & cov_P > 10]

fwrite(gene_meth, "gene_level_TE_CHH.tsv", sep = "\t")
